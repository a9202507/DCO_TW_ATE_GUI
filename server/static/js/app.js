...
function updateStatusDisplay(instrumentType, data) {
    const displayDiv = document.getElementById(`status-display-${instrumentType}`);
    if (!displayDiv) return;

    let html = '';
    if (instrumentType === 'power-supply') {
        html = `
            <div class="status-item"><span class="status-label">State</span><span class="status-value">${data.output || 'N/A'}</span></div>
            <div class="status-item"><span class="status-label">Voltage</span><span class="status-value">${data.voltage || '0.00'} V</span></div>
            <div class="status-item"><span class="status-label">Current</span><span class="status-value">${data.current || '0.00'} A</span></div>
        `;
...
async function checkClientStatus() {
    try {
        ...
        document.getElementById('connectionStatus').textContent = clientInfo.status === 'connected' ? '已連線' : '連線錯誤';
...
async function detectInstruments() {
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            showGlobalStatus(`✅ 成功偵測到 ${result.instruments.length} 個儀器`, 'success');
...
async function controlInstrument(instrumentType, action) {
    const addressSelect = document.getElementById(`address-${instrumentType}`);
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            showStatus(instrumentType, `❌ 操作失敗: ${result.detail || result.message}`, 'error');
        }
    } catch (error) {
        showStatus(instrumentType, `❌ 操作時發生網路錯誤`, 'error');
    } finally {
        setTimeout(() => {
            panel.querySelectorAll('button').forEach(b => b.disabled = address === '');
        }, 1000);
    }
}
...
function plotWaveform(instrumentType, data) {
    ...
    if (!data || !data.x || !data.y) {
        container.innerHTML = '<p style="color: red; text-align: center;">無效的波形資料</p>';
        return;
    }
...
function displayInstruments(instruments) {
    ...
    if (!instruments || instruments.length === 0) {
        instrumentList.innerHTML = '<div class="no-instruments">未發現任何 GPIB 儀器。</div>';
        return;
    }
    ...
    instrumentTypes.forEach(type => {
        ...
        if (select && panel) {
            select.addEventListener('change', () => {
                ...
                panel.querySelectorAll('button').forEach(b => b.disabled = !hasValue);
                if (hasValue) {
